...
.. py:class:: ctx.dist_nmax_rho(rho, k)
...
.. method:: dist_nmax_rho.pdf(x)
    ...
    .. math:: \text{pdf}_X(x) = f_{\mathrm{nmax\_corr}}(x, \rho, k)  = \frac{k}{b} \int_{-\infty}^\infty \left( \Phi(z_1) \right)^{k-1} \phi(z_1) \: \phi(y) \: \mathrm{d} y, \quad \text{where}
...
.. method:: dist_nmax_rho.cdf(x)
    ...
    .. math:: \text{cdf}_X(x)= F_{\mathrm{nmax\_corr}}(x, \rho, k)  = \int_{-\infty}^\infty \left[\Phi \left(\frac{x + \sqrt{\vert \rho \vert} y} {\sqrt{1-\rho}} \right) \right]^n \phi(y) \mathrm{d} y 
...
.. method:: dist_nmax_rho.k_x(t, k = 0)
...
.. method:: dist_nmax_rho.moments(k)
...
.. method:: dist_nmax_rho.cumulants(k)
