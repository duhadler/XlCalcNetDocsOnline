...
.. py:class:: ctx.dist_nair_t(rho, k, nu)

where ``ctx`` is ``fpm``, ``mpm``, ``ipm``, ``dec``, ``gmp`` or ``apm``.

The  Nair `t`-distribution with common correlation `\rho`,  `k \ge 2` groups (including control group) and error degrees of freedom `\nu` is a continuous distribution with the support interval `(-\infty, +\infty)`.
...
has a Nair `t`-distribution with `k` and `n` degrees of freedom.
...
.. method:: dist_nair_t.pdf(x)

    Returns `\text{pdf}_X(x)`, the probability density function (pdf) of a random variable `X`, following a Nair t-distribution:
...
.. method:: dist_nair_t.cdf(x)

    Returns `\text{cdf}_X(x)`, the cumulative distribution function (cdf) of a random variable `X`, following a Nair t-distribution:
...
.. method:: dist_nair_t.sf(x)

    Returns `\text{sf}_X(x)`, the survival function function (sf) of a random variable `X`, following a Nair t-distribution:
...
.. method:: dist_nair_t.qtf(q)

    Returns `\text{qtf}_X(x)`, the quantile function function (qtf) of a random variable `X`, following a Nair t-distribution:
...
.. method:: dist_nair_t.isf(q)

    Returns `\text{isf}_X(q)`, the inverse survival function function (isf) of a random variable `X`, following a Nair t-distribution:
...
.. method:: dist_nair_t.c_x(t)

    Returns `C_X(t)`, the characteristic function of a random variable `X`, following a Nair t-distribution:
...
.. method:: dist_nair_t.moments(k)

    Returns the first `j` raw moments, `\mu_j, j = 1 \ldots k`, of a random variable `X`, following a Nair t-distribution. The rth moments only exists for `n_2 > 2r`.
...
.. method:: dist_nair_t.cumulants(k)

Returns the first `j` cumulants, `\kappa_j, j = 1 \ldots k`, of a random variable `X`, following a Nair t-distribution. The cumulants are calculated from the moments.
