...
.. py:class:: ctx.dist_lrt_x0_s0(p, n)

    where ``ctx`` is ``fpm``, ``mpm``, ``ipm``, ``dec``, ``gmp`` or ``apm``.

    The distribution of the modified  (LRT) for a given covariance matrix and mean vector is a continuous probability distribution with `p \ge 1` predictor variables, error degress of freedom  `n \ge 1`, and the support interval `(0,1)`.
    See also :cite:t:`Anderson2003`, :cite:t:`Davis1971`.  For tables see  Nagarsenker 1974


    The likelihood criterion `\lambda_2` for testing the hypothesis that a sample of size `N` is drawn from 
    a `p`-variate normal population with a given covariance matrix `\boldsymbol{\Sigma}_0` and mean vector
    `\boldsymbol{\mu}_0` is given by

    .. math::

        \lambda_2 = \left(\frac{e}{N} \right)^{\tfrac{1}{2}pN} \left|\boldsymbol{S \Sigma}_0^{-1}\right|^{\tfrac{1}{2}N} \exp \left(-\tfrac{1}{2}\left[\text{tr} \left(\boldsymbol{S \Sigma}_0^{-1}\right) +N(\boldsymbol{\bar{x}}-\boldsymbol{\mu}_0)' \boldsymbol{\Sigma}_0 (\boldsymbol{\bar{x}}-\boldsymbol{\mu}_0)\right]\right)

    where `\boldsymbol{S}` is the sample covariance matrix and `\boldsymbol{\bar{x}}` is the sample mean vector.
...
.. method:: dist_lrt_x0_s0.pdf(x)
    ...
    .. code-block:: python
        ...
        >>> mp.dps = 30
        >>> mu = 0; sigma = 1; x = 3; 
        >>> print ("pdf: ", lrt_x0_s0(mu, sigma).pdf(x))
...
.. method:: dist_lrt_x0_s0.cdf(x)
    ...
    .. code-block:: python
        ...
        >>> mp.dps = 30
        >>> mu = 0; sigma = 1; x = 3; 
        >>> print ("cdf: ", lrt_x0_s0(mu, sigma).pdf(x))
        6.3563523462564525615615615614561356E-20
...
.. method:: dist_lrt_x0_s0.sf(x)
    ...
    .. code-block:: python
        ...
        >>> mp.dps = 30
        >>> mu = 0; sigma = 1; x = 3; 
        >>> print (" sf: ", lrt_x0_s0(mu, sigma).pdf(x))
...
.. method:: dist_lrt_x0_s0.qtf(q)
    ...
    .. code-block:: python
        ...
        >>> mp.dps = 30
        >>> mu = 0; sigma = 1; q = 0.3; 
        >>> print ("qtf: ", lrt_x0_s0(mu, sigma).qtf(q))
...
.. method:: dist_lrt_x0_s0.isf(q)
    ...
    .. code-block:: python
        ...
        >>> mp.dps = 30
        >>> mu = 0; sigma = 1; q = 0.3; 
        >>> print ("isf: ", lrt_x0_s0(mu, sigma).isf(q))
...
.. method:: dist_lrt_x0_s0.c_x(t)
    ...
    .. math::  C_X(t) = \frac{\left(2e/N\right)^{-it \cdot pN}}{(1-2it)^{-\tfrac{1}{2}pN(1-2it)}}   \prod_{j=1}^p  \frac{ \Gamma\left[\tfrac{1}{2}N(1-2it)- j/2\right]}{ \Gamma \left[\tfrac{1}{2}(N-j)\right]}
    ...
    .. code-block:: python
        ...
        >>> mp.dps = 30
        >>> mu = 0; sigma = 1; t = 0.3; 
        >>> print ("c_x: ", lrt_x0_s0(mu, sigma).c_x(t))
        6.3563523462564525615615615614561356E+00
...
.. method:: dist_lrt_x0_s0.m_x(t)

    Returns `M_X(t)`, the moment generating function of a random variable `X`, following the distribution of Box's LRTs:


    .. math::  M_X(t) = \frac{\left(2e/N\right)^{-t \cdot pN}}{(1-2t)^{-\tfrac{1}{2}pN(1-2t)}}   \prod_{j=1}^p  \frac{ \Gamma\left[\tfrac{1}{2}N(1-2t)- j/2\right]}{ \Gamma \left[\tfrac{1}{2}(N-j)\right]}
...
.. method:: dist_lrt_x0_s0.k_x(s, k = 0)

    Returns `K_X(s)`, the cumulant generating function, and its `j^{\text{th}}` derivatives, `K_X^{(j)}(s), j = 1 \ldots k`, 
    of a random variable `X`, following the distribution of Box's LRTs:


    .. math::  M_{X}(t) = \log(M_{X}(t))
    ...
    .. code-block:: python
        ...
        >>> print ("c_x: ", lrt_x0_s0(mu, sigma).k_x(t, k))
        6.3563523462564525615615615614561356E+00
...
.. method:: dist_lrt_x0_s0.moments(k)
    ...
    .. code-block:: python
        ...
        >>> mp.dps = 30
        >>> mu = 0; sigma = 1; k = 6;
        >>> print ("saddlepoint: ", lrt_x0_s0(mu, sigma).moments(k))
        6.3563523462564525615615615614561356E+00
...
.. method:: dist_lrt_x0_s0.cumulants(k)
    ...
    .. code-block:: python
        ...
        >>> mp.dps = 30
        >>> mu = 0; sigma = 1; k = 6;
        >>> print ("saddlepoint: ", lrt_x0_s0(mu, sigma).cumulants(k))
        6.3563523462564525615615615614561356E+00
...
.. math::  f=\tfrac{1}{2} p(p+3); \quad \rho=1-\frac{2p^2+9p+11}{6N(p+3)}

.. math::  \omega_r = \frac{2(-1)^r}{r(r+1)(r+2) \rho^r} \sum_{s=1}^{r+1} \binom{r+2}{s+1} (1-\rho-N^{-1})^{r+1-s} \frac{\delta_s+p(s+1)/2^{s+1}}{(\tfrac{1}{2}N)^{s-1}}, 
...
.. method:: ctx.lrt_x0_vc0_bd(q, f, rho, omega)
    ...
    .. math::  f=\tfrac{1}{2} p(p+3); \quad \rho=1-\frac{2p^2+9p+11}{6N(p+3)}; \quad \delta_{s,p} = -\frac{s+1}{2} \sum_{j=0}^{p-1} B_s(-j/2)

    .. math::  \omega_r = \frac{2(-1)^r}{r(r+1)(r+2) \rho^r} \sum_{s=1}^{r+1} \binom{r+2}{s+1} (1-\rho-N^{-1})^{r+1-s} \frac{\delta_{s,p} + p(s+1)/2^{s+1}}{(\tfrac{1}{2}N)^{s-1}}, 

    where `B_s(\cdot)` is the Bernoulli polynomial of degree `s`. . For tables see Nagarsenker 1974



.. method:: ctx.lrt_x0_vc0_bd_inv(q, f, rho, omega)
