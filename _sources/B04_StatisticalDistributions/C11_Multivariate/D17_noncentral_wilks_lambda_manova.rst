...
.. _rst_dist_wilks_lambda_glm: 

Noncentral Distribution of Wilks' `\Lambda`:  MANOVA
-------------------------------------------------------------------------------


.. py:class:: ctx.dist_wilks_lambda_glm(p, m, n, omega)

    where ``ctx`` is ``fpm``, ``mpm``, ``ipm``, ``dec``, ``gmp`` or ``apm``.

    The noncentral Wilks’ `\Lambda` distribution under the GLM alternative, with `p \ge 1` predictor variables, error degress of freedom `m \ge 1` and `n \ge 1`, noncentrality parameter `\Omega` with diagonal entries `\omega_{jj} \in (0,\infty)` is a continuous probability distribution  with the support interval `(0,1)`.
    See also :cite:t:`Anderson2003`, :cite:t:`Muirhead1982`, :cite:t:`Butler2007`, :cite:t:`Butler2002a`, :cite:t:`Butler2005`, :cite:t:`Fujikoshi1970`, :cite:t:`Fujikoshi1973`,  :cite:t:`Fujikoshi1974`, :cite:t:`Lee1971a`, :cite:t:`Lee1971b`, :cite:t:`Walster1980`, :cite:t:`CharfunDis1003`.
...
.. method:: dist_wilks_lambda_glm.pdf(x)

    Returns `\text{pdf}_X(x)`, the probability density function (pdf) of a random variable `X`, following the noncentral distribution of Wilks' Lambda (GLM):
...
.. method:: dist_wilks_lambda_glm.cdf(x)

    Returns `\text{cdf}_X(x)`, the cumulative distribution function (cdf) of a random variable `X`, following the noncentral distribution of Wilks' Lambda (GLM):
...
.. method:: dist_wilks_lambda_glm.sf(x)
...
.. method:: dist_wilks_lambda_glm.qtf(q)
...
.. method:: dist_wilks_lambda_glm.isf(q)
...
.. method:: dist_wilks_lambda_glm.c_x(t)
    ...
    .. math:: C_X(t) = \frac{\Gamma_p(n/2  -it)\Gamma_p((n + m)/2)}{\Gamma_p(n/2)\Gamma_p((n + m)/2  -it)} {}_1F_1\left( -it ;\frac{n + m}{2}+  -it ; -\frac{1}{2}\Omega\right).
...
.. method:: dist_wilks_lambda_glm.m_x(t)
    ...
    .. math:: M_X(t) = \left[ \frac{\Gamma_p(n/2 + s)\Gamma_p((n + m)/2)}{\Gamma_p(n/2)\Gamma_p((n + m)/2 + s)} {}_1F_1 \left(s,\tfrac{1}{2}(n+m)+s,-\tfrac{1}{2}\Omega\right) \right].
...
.. method:: dist_wilks_lambda_glm.k_x(s, k = 0)

    Returns `K_X(s)`, the cumulant generating function, and its `j^{\text{th}}` derivatives, `K_X^{(j)}(s), j = 1 \ldots k`, 
    of a random variable `X`, following the noncentral distribution of Wilks' Lambda (GLM):

    .. math:: K_X(s) = \log \left[ \frac{\Gamma_p(n/2 + s)\Gamma_p((n + m)/2)}{\Gamma_p(n/2)\Gamma_p((n + m)/2 + s)} {}_1F_1 \left(s,\tfrac{1}{2}(n+m)+s,-\tfrac{1}{2}\Omega\right) \right].
    ...
    .. math::
       ...
       \begin{eqnarray}
        ...
        & + & \frac{\partial}{\partial s} \log \left[ {}_1F_1 \left(s,\tfrac{1}{2}(n+m)+s,-\tfrac{1}{2}\Omega\right) \right] \nonumber , 
       \end{eqnarray}

    where  `\Gamma_p(\cdot)` is the multivariate gamma function , 
    `\psi(\cdot)` is the digamma function , and `{}_1F_1(\cdot,\cdot,X)` is 
...
.. method:: dist_wilks_lambda_glm.moments(k)
    ...
    .. math:: \mu'_X(r) =  \frac{\Gamma_p(n/2 + s)\Gamma_p((n + m)/2)}{\Gamma_p(n/2)\Gamma_p((n + m)/2 + s)} {}_1F_1\left(s ;\frac{n + m}{2}+ s ; -\frac{1}{2}\Omega\right).
...
.. method:: dist_wilks_lambda_glm.cumulants(k)

    Returns the first `j` cumulants, `\kappa_j, j = 1 \ldots k`, of a random variable `X`, 
    following the noncentral distribution of Wilks' Lambda (GLM)
...
.. method:: ctx.wilks_lambda_glm_gp(x, p, m, n, Omega, cdf=True, method='default')
...
.. method:: ctx.wilks_lambda_glm_spa(x, n, results='c')
...
.. method:: ctx.wilks_lambda_glm_inv(x, n, results='qtf')
