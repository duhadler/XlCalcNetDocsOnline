...
.. method:: ctx.nsum(f, *intervals, **options)

    where ``ctx`` is ``dec``, ``mpm``, or ``gmp``.

    See also: Wikipedia :cite:p:`WikipediaAlg70`.
    ...
    **Examples with Levin transformation**
    ...
    numerical cancellation.
        ...
        >>>  # the following is the symbolic expression for the integral
        >>> exact = mp.exp(mp.one / (32 * z)) * mp.besselk(mp.one / 4, \
        ...   mp.one / (32 * z)) / (4 * mp.sqrt(z * mp.pi))
        >>> w = mp.nsum(lambda n: (-z)**n * mp.fac(4 * n) / (mp.fac(n) * mp.fac(2 * n) * (4 ** n)),
        ...   [0, mp.inf], method = "levin", levin_variant = "t", workprec = 8*mp.prec, 
    ...
    **Multidimensional sums**
    ...
    reductions to single-dimensional series (see MathWorld :cite:p:`WolframFun2000`)::
    ...
    in three dimensions (see MathWorld :cite:p:`WolframFun2001`). The defining sum converges very
...
.. method:: ctx.sumem(f, interval, tol=None, reject=10, integral=None, adiffs=None, bdiffs=None, verbose=False, error=False, _fast_abort=False)
    ...
    .. math ::
        ...
        S \sim \int_a^b f(x) \,\mathrm{d} x + \frac{f(a)+f(b)}{2} +
...
.. method:: ctx.sumap(f, interval, integral=None, error=False)
    ...
    .. math ::
        ...
        \sum_{k=0}^{\infty} f(k) = \int_0^{\infty} f(t) \mathrm{d} t + \frac{1}{2} f(0) +
            i \int_0^{\infty} \frac{f(it)-f(-it)}{e^{2\pi t}-1} \mathrm{d} t.
...
.. method:: ctx.nprod(f, interval, nsum=False, **kwargs)
    ...
    **Examples**
    ...
    examples are taken from MathWorld (see MathWorld :cite:p:`WolframFun2002`).
...
.. method:: ctx.limit(f, x, direction=1, exp=False, **kwargs)
...
.. method:: ctx.richardson(seq)

    where ``ctx`` is ``dec``, ``mpm``, or ``gmp``.

    See also: Wikipedia :cite:p:`WikipediaAlg71` and :cite:t:`Bender1999`, pp. 375-376.
...
.. method:: ctx.shanks(seq, table=None, randomized=False)

    where ``ctx`` is ``dec``, ``mpm``, or ``gmp``.

    See also: Wikipedia :cite:p:`WikipediaAlg72`, MathWorld :cite:p:`WolframAlg52`.
    ...
    **The Shanks transformation**
    ...
    The Shanks transformation is defined as follows (see :cite:t:`Bender1999`, pp. 368-375): given
    ...
    the cost of requiring dummy elements in the table. See :cite:t:`GravesMorris2000` for
    details.
...
.. method:: ctx.levin(seq, table=None, randomized=False)
    ...
    See also:  :cite:t:`Weniger1989`,  :cite:t:`Sidi2003`,  :cite:t:`Homeier2000`.
    ...
    **Examples**
    ...
    exponential integral (see also :cite:t:`Sidi2003` p.373). The Sidi-S transform works best here:
...
.. method:: ctx.cohen_alt(seq, table=None, randomized=False)

    where ``ctx`` is ``dec``, ``mpm``, or ``gmp``.


    This interface implements the convergence acceleration of alternating series
    as described in :cite:t:`Cohen2000`.


    This series transformation works only well if the
