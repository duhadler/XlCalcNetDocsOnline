...
Python code for ``gmp`` and ``dec`` is available in ``Test_Python.numpy.py``.
...
.. method:: ctx.polyval(coeffs, x, derivative=False)
...
.. method:: ctx.polyroots(coeffs, maxsteps=50, cleanup=True, extraprec=10, error=False, roots_init=None)
    ...
    **Algorithm**
    ...
    :func:`~mpmath.polyroots` implements the Durand-Kerner method (see Wikipedia :cite:p:`WikipediaAlg10`), which
