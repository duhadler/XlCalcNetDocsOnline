...
.. py:class:: ctx.dist_box_mvn(p, k, ni)

    where ``ctx`` is ``fpm``, ``mpm``, ``ipm``, ``dec``, ``gmp`` or ``apm``.

    The distribution of Box's test for same multivariate normal distributions is a continuous probability distribution with `p \ge 1` predictor variables, error degress of freedom  `n_i \ge 1`, and the support interval `(0,1)`.
...
.. method:: dist_box_mvn.k_x(s, k = 0)
    ...
    The cumulant generating function and the `j` th derivative of the cumulant generating function ` `M = -2 \log(W)` are given by 


    .. math:: K_X(t) = 2 \left( \sum_{i=1}^m x_i \log(x_i) - \sum_{r=1}^k y_r \log(y_r) -  \sum_{l=1}^m x_l \psi(x_l(1-2t)+\xi_l) + \sum_{r=1}^k y_r \psi(y_r+\eta_r) \right), \quad \text{and }

    .. math:: K_X^{(j)}(t) =  \sum_{i=1}^m (-2x_l)^j \psi^{(j-1)}(x_l(1-2t)+\xi_l) - \sum_{r=1}^k (-2y_r(1-2t))^j \psi^{(j-1)}(y_r(1-2t)+\eta_r), \quad j \ge 2,
...
.. method:: dist_box_mvn.cumulants(k)
    ...
    .. math:: \kappa_1 = 2 \left( \sum_{i=1}^m x_i \log(x_i) - \sum_{r=1}^k y_r \log(y_r) -  \sum_{i=1}^m x_i \psi(x_i+\xi_i) + \sum_{r=1}^k y_r \psi(y_r+\eta_r) \right), \quad \text{and }

    .. math:: \kappa_j =  \sum_{i=1}^m (-2x_i)^j \psi^{(j-1)}(x_i+\xi_i) - \sum_{r=1}^k (-2y_r)^j \psi^{(j-1)}(y_r+\eta_r), \quad j \ge 2,
...
.. method:: ctx.box_means_cov_bd(x, f, rho, omega)
...
.. method:: ctx.box_means_cov_bd_inv(q, f, rho, omega)
