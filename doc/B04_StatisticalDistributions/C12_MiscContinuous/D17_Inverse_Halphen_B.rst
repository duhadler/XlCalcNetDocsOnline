...
.. _rst_dist_halphen_ib: 

Halphen IB distribution
-------------------------------------------------------------------------------


.. py:class:: ctx.dist_halphen_ib(a, b)

    where ``ctx`` is ``dec``, ``mpm``, ``ipm``, ``fpm``, ``gmp`` or ``arb``.

    The Halphen IB (Inverse Halphen B) distribution is a continuous probability distribution with parameters `a > 0` and  `b > 0`, and the support interval `(0, +\infty)`.
...
.. method:: dist_halphen_ib.k_x(t, k = 0)
...
.. method:: dist_halphen_ib.moments(k)
...
.. method:: dist_halphen_ib.cumulants(k)
