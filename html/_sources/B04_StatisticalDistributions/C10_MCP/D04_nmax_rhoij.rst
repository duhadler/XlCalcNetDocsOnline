...
.. _rst_dist_nmax_rhoij: 

Normal maximum distribution, `\rho_{ij, i \ne j} = \lambda_i \lambda_j`
-------------------------------------------------------------------------------


.. py:class:: ctx.dist_nmax_rhoij(rho, k)

    where ``ctx`` is ``fpm``, ``mpm``, ``ipm``, ``dec``, ``gmp`` or ``apm``.

    The distribution of the maximum of `k \ge 2` correlated standard normal variates,  with common correlation `0 \le \rho \le 1` is a continuous distribution with support interval `(-\infty, +\infty)`.
...
.. method:: dist_nmax_rhoij.pdf(x)
    ...
    .. math:: \text{pdf}_X(x) = f_{\text{nmaxrhoij}}(x, \rho, k)  = \frac{k}{b} \int_{-\infty}^\infty \left( \Phi(z_1) \right)^{k-1} \phi(z_1) \: \phi(y) \: \mathrm{d} y, \quad \text{where}

    .. math:: z_1 = \frac{x+a}{b}, \quad  a = y \sqrt{|\rho|}, \quad  b = \sqrt{1-\rho}.
...
.. method:: dist_nmax_rhoij.cdf(x)
    ...
    .. math:: \text{cdf}_X(x)= F_{\text{nmaxrhoij}}(x, \rho, k)  = \int_{-\infty}^\infty \left[\Phi \left(\frac{x + \sqrt{\vert \rho \vert} y} {\sqrt{1-\rho}} \right) \right]^n \phi(y) \mathrm{d} y 
...
.. method:: dist_nmax_rhoij.sf(x)
    ...
    .. math:: \text{sf}_X(x) = 1 -  \int_{-\infty}^\infty \left[\Phi \left(\frac{x + \sqrt{\vert \rho \vert} y} {\sqrt{1-\rho}} \right) \right]^n \phi(y) \mathrm{d} y 
...
.. method:: dist_nmax_rhoij.k_x(t, k = 0)
...
.. method:: dist_nmax_rhoij.moments(k)
...
.. method:: dist_nmax_rhoij.cumulants(k)
